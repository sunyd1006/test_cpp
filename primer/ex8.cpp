...
// g11 Sales_data.cpp ex8.cpp -o main && ./main "./ex8io/test_book.txt" "./ex8io/test_book_out.txt"
int main(int argc, char **argv) {
    std::cout << "\n case: 打印 cpp 变量类型 8.1.2" << std::endl;
    testClassName item;
    ...
    std::cout << "\n 章节 ------------------ 8.2 文件输入输出" << std::endl;
    std::cout << "\n习题 ------ 8.4" << std::endl;
    std::vector<string> fileContent;
    readFileToVec("./ex8io/test_file.txt", fileContent);
    for (const auto &range : fileContent) {
        std::cout << range << std::endl;
    }

    std::cout << "\n习题 ------ 8.5 从命令行读取参数" << std::endl;
    ifstream ifs(argv[1]);  // ./ex8io/test_book.txt
    cout << "argc len: " << argc << " args: " << argv[1] << endl;
    startMain(ifs, std::cout);
    ifs.close();

    std::cout << "\n习题 ------ 8.7 从命令行读取参数 读到文件 输出到文件" << std::endl;
    std::ifstream ifs1(argv[1]);  // ./ex8io/test_book.txt
    std::ofstream ofs1(argv[2]);  // ./ex8io/test_book_out.txt
    cout << "argc len: " << argc << " args: " << argv[2] << endl;
    startMain(ifs1, ofs1);

    std::cout << "\n习题 ------ 8.8 从命令行读取参数 读到文件 追加到文件" << std::endl;
    std::ifstream ifs2(argv[1]);  // ./ex8io/test_book.txt
    std::ofstream ofs2(argv[2], ofstream::app);  // ./ex8io/test_book_out.txt
    cout << "argc len: " << argc << " args: " << argv[2] << endl;
    startMain(ifs2, ofs2);
    ...
    std::fstream f_person_in("./ex8io/test_person.txt");
